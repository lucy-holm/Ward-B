...
The client vocabulary also supports actionable playtest signals:
`puzzle_step` (`puzzle`, `step`), `noise` (`source`),
`investigation_started`/`investigation_ended`, and
`hazard_warning`/`hazard_activated`/`hazard_avoided`/`hazard_caught`.
Checkpoint lifecycle (`checkpoint_saved`, `checkpoint_save_failed`,
`checkpoint_restored`, `checkpoint_continue`) and pursuit
(`pursuit_stalled`, `pursuit_recovered`) are included for the resume and
pressure analyses.
These names are additive and the Worker preserves unknown future names. The
client and Worker bound event strings, arrays, object keys, and nesting before
storage, while retaining the complete bounded event JSON in `data`.
...
> the matching header to `flush()` in `godot/autoload/telemetry.gd`. The
> `src/` path is the frozen Three.js archive. Never change one without the
> other.
> without the other.
...
`BuildConfig.TELEMETRY_ENDPOINT` (see `godot/core/build_config.gd`; the
checked-in value is empty, which makes every local event log to the console
...
In the main Ward B repo (not this directory), pass the `*.workers.dev` URL
from step 6 to the Godot export's build configuration (`BuildConfig.TELEMETRY_ENDPOINT`,
generated by `godot/tools/write_build_config.sh`). The checked-in config must
remain empty; the itch export workflow is the only place that should inject
the endpoint. That's outside this directory's ownership; hand this URL to
whoever's wiring the client side.
...
Build a report from a read-only D1 JSON export. This does not contact the
Worker or write any database:

```
npx wrangler d1 execute wardb-telemetry --remote --json \
  --command='SELECT * FROM events ORDER BY id' > events.json
node report.mjs events.json > report.json
# or a human-readable review copy
node report.mjs --markdown events.json > report.md
```

The checked-in Worker contract test is offline and uses an in-memory D1:

```
node test_ingest.mjs
```

The report includes a fresh-run funnel with non-completing sessions and a
separate resumed funnel (checkpoint continuations are not counted as fresh-run
churn), per-room active time and completion, catches/shifts,
noise/investigation/hazard/checkpoint counts, puzzle-stall candidates, and
platform/performance segments. It reports sample
sizes and 95% Wilson intervals; segments below ten sessions are marked
`adequate: false`. A `quit` is an observed terminal close path and does not
establish frustration. A session with no `quit` or `game_complete` is labelled
an inferred abandonment-or-missing-terminal-event because a crash, network
loss, or export cutoff can produce the same shape. Recent unresolved sessions
are reported separately as right-censored using a five-minute observation
window.
